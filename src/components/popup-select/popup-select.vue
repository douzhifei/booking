...
export default {
  ...
  methods: {
    ...
    setWheelItemDeg (index) {
      return {
        transform: `rotate3d(1, 0, 0, ${-index * 10 % 360}deg) translate3d(0px, 0px, 275px)`
      }
    },
    ...
    setListTransform (translateY = 0, marginTop = 0, type, time = 1000) {
      if (type === 'end') {
        ...
        this.$refs.list.style.webkitTransform = `translateY(${translateY - this.spin.branch * 48}px)`
    ...
    setStyle (move, type, time) {
      ...
      let spinAim = Math.round(updateDeg / 10)
    ...
    /* 获取选中值 */
    getPickValue (move) {
      let index = Math.round(Math.abs(move / 48))
